...
function Notes(){
		...
		if (paket.err===0){
			for (x in paket.data) {
				html+='<tr>'
					...
					+'<td><button type="button" id="btn_edit" onclick="objModul.editData(\''+paket.data[x].note_blok+'\');">Edit</button></td>'
					+'</tr>';
			}
		}
		html+='</table>';
	...
	function editData(blok_id){
		...
		html='<button type="button" id="btn_back" onclick="objModul.readData();">Back</button>'
			...
			+'<button type="button" id="btn_delete" onclick="objModul.deleteExecute(\''+blok_id+'\');">Delete</button>';
		btn.innerHTML=html;	
		msg.innerHTML='';
	...
	function searchData(txt){
		...
		html='<input type="text" value="'+txt+'" id="txt_search" placeholder="Type text to search ..." onfocus="this.select();">'
			+'<button type="button" id="btn_search" onclick="objModul.searchExecute();">Start Search</button>';
		app.innerHTML=html;
