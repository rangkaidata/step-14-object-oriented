...
function Home(){
		...
		if (paket.err==0){
					...
					if (paket.data[x].menu_parent=='home'){
						if (paket.data[x].company_blok!=''){
							if (paket.data[x].menu_code=='folder'){
								html+='<button type="button" id="folder" onclick="objHome.subMenuOpen(\'company\')">'+paket.data[x].company_name+'</button>';
							}
							else{
								html+='<button type="button" onclick="objHome.subMenuOpen(\''+paket.data[x].menu_code+'\')">'+paket.data[x].menu_name+'</button>';
							}
						}
						else{
							html+='<button type="button" onclick="objHome.subMenuOpen(\''+paket.data[x].menu_code+'\')">'+paket.data[x].menu_name+'</button>';
			...
			if (paket.data[x].company_blok!=''){
				if (login_blok_master===null){
					html+='<button type="button" onclick="objHome.closeFolder()">Close Folder</button>';
				}
			}
			
			// user
			html+='<button type="button" onclick="load_modul(\'logout\')">'+data_login.data[0].user_name+'</button>';
			menu.innerHTML = html;	
	...
	function subMenuOpen(menu_code){
		...
		var tipe;
		var html;
		html='<button onclick="objHome.subMenuClose();">Close</button>'
